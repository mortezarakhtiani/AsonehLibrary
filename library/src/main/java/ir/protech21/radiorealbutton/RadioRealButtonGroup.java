            ...
            if (lastPosition != position) {
                if (animationType == ANIM_TRANSLATE_X) {
                    if (initialPosition != -1)
                        v_selectors.get(0).setVisibility(INVISIBLE);
                    v_selectors.get(0).setVisibility(VISIBLE);
                    lastPosition = initialPosition = 0;
                }
                setPosition(0, false);
            }
        }
...
//            selectorContainer.setLayoutDirection(LAYOUT_DIRECTION_RTL);
//        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            container.setLayoutDirection(LAYOUT_DIRECTION_RTL);
        }
    ...
    public void update(){
        if (v_selectors.isEmpty()){
            return;
        }
        v_selectors.get(0).getLayoutParams().width=buttons.get(0).getWidth();
        v_selectors.get(0).setX(buttons.get(0).getX());
        v_selectors.get(0).requestLayout();
    }
    ...
    private void createSelectorItem(int position, RadioRealButton button) {
        ...
        view.setLayoutParams(new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, height));
    ...
    private void initButtonListener(RadioRealButton button, final int position) {
        ...
        button.setOnClickListener(v -> makeSelection(position, true, hasAnimation));
    ...
    public void moveSelector(int toPosition, boolean hasAnimation, boolean enableDeselection) {
...
//        float value = buttons.get(toPosition).getMeasuredWidth() * toPosition + dividerSize * position;
//        ObjectAnimator animator = createAnimator(v_selectors.get(initialPosition), property, value, false, hasAnimation);
//        animator.start();
        v_selectors.get(initialPosition).animate().translationX(buttons.get(toPosition).getX()).setDuration(animateSelectorDuration).start();

        ValueAnimator animato= ValueAnimator.ofInt(buttons.get(lastPosition).getMeasuredWidth(),buttons.get(toPosition).getMeasuredWidth()).setDuration(animateSelectorDuration);
        int finalToPosition = toPosition;
        animato.addUpdateListener(animation -> {
            View view = v_selectors.get(initialPosition);
            view.getLayoutParams().width= (int) animation.getAnimatedValue();
            view.requestLayout();
            lastPosition= finalToPosition;
        });
        animato.start();
//        onPositionChangedListener.onPositionChanged(buttons.get(toPosition), toPosition, lastPosition);
    }
